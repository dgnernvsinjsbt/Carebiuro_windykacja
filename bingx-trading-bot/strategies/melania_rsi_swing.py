"""
MELANIA RSI Swing Strategy - CORRECTED RSI
...
Entry: RSI(14) crosses above 25 (LONG) or below 75 (SHORT)
Limit: 1.5% offset from signal price (wait max 5 bars)
Exit: 1.0x ATR stop loss, 3.0x ATR take profit, or RSI reversal
"""
...
class MELANIARSISwingStrategy(BaseStrategy):
    """MELANIA RSI Mean Reversion Strategy with Limit Orders"""

    def __init__(self, config: Dict[str, Any], symbol: str = 'MELANIA'):
        super().__init__('melania_rsi_swing', config, symbol)

        # RSI parameters
        self.rsi_low = config.get('rsi_low', 25)
        self.rsi_high = config.get('rsi_high', 75)
