"""
BTC RSI Swing Strategy (Limit Orders)

**Return/DD Ratio: 8.34x** ⭐
+67.18% return, -8.06% DD, 95 trades (from 90-day backtest)

Entry Conditions:
- RSI 30/65: LONG when RSI crosses above 30, SHORT when RSI crosses below 65
- Limit Order: 0.5% offset (LONG: -0.5% below signal, SHORT: +0.5% above signal)
- Max wait: 5 bars for limit fill (else cancel)

Exit Conditions:
- Stop Loss: 2.0x ATR from entry
- Take Profit: RSI 65 for LONG, RSI 30 for SHORT
...
class BTCRSISwingStrategy(BaseStrategy):
    """
    BTC RSI Mean Reversion with Limit Orders
    ...
    def __init__(self, config: Dict[str, Any], symbol: str = 'BTC-USDT'):
        super().__init__('btc_rsi_swing', config, symbol)

        # Entry parameters
        self.rsi_low = config.get('rsi_low', 30)
        self.rsi_high = config.get('rsi_high', 65)
        self.limit_offset_pct = config.get('limit_offset_pct', 0.5)
    ...
    def analyze(self, df_1min: pd.DataFrame, df_5min: Optional[pd.DataFrame] = None, df_1d: Optional[pd.DataFrame] = None) -> Optional[Dict[str, Any]]:
        ...
        """
        if len(df_1min) < 50:  # Need enough data for RSI
            return None
        ...
        # Calculate SL (from limit price, not signal price)
        atr = current['atr']
        if direction == 'LONG':
            stop_loss = limit_price - (self.stop_atr_mult * atr)
            take_profit = None  # RSI-based exit, managed by exit logic
        else:  # SHORT
            stop_loss = limit_price + (self.stop_atr_mult * atr)
            take_profit = None
        ...
        return {
            ...
            'stop_loss': stop_loss,
            'take_profit': take_profit,  # None - RSI exit
            'current_bar': current_bar,
            'max_wait_bars': self.max_wait_bars,
            'pattern': pattern,
            'confidence': 0.61,  # 61.4% WR from backtest
            'atr': atr,
            'rsi': current_rsi,
