"""
CRV RSI Swing Strategy - CORRECTED RSI
...
Entry: RSI(14) crosses above 25 (LONG) or below 70 (SHORT)
Limit: 2.0% offset from signal price (wait max 5 bars)
Exit: 1.0x ATR stop loss, 1.0x ATR take profit, or RSI reversal
"""
...
class CRVRSISwingStrategy(BaseStrategy):
    """CRV RSI Mean Reversion Strategy with Limit Orders"""

    def __init__(self, config: Dict[str, Any], symbol: str = 'CRV'):
        super().__init__('crv_rsi_swing', config, symbol)

        # RSI parameters
        self.rsi_low = config.get('rsi_low', 25)
        self.rsi_high = config.get('rsi_high', 70)
        ...
        # Exit parameters
        self.stop_atr_mult = config.get('stop_atr_mult', 1.0)
        self.tp_atr_mult = config.get('tp_atr_mult', 1.0)
    ...
    def generate_signals(self, df: pd.DataFrame, current_positions: list) -> Optional[Dict[str, Any]]:
        ...
        # LONG signal: RSI crosses above 25
