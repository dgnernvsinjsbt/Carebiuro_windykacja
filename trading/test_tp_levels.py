#!/usr/bin/env python3
"""Test different TP levels: flat % vs ATR-based"""
import pandas as pd
import numpy as np
...
def test(tp_type, tp_value):
    """
    tp_type: 'flat' or 'atr'
    tp_value: if flat, it's %, if atr, it's ATR multiplier
    """
    limit_atr_offset = 0.8  # Fixed optimal
    rsi_trigger = 72
    lookback = 5
    max_wait_bars = 20
        ...
        if limit_pending:
            ...
            if row['high'] >= limit_price:
                ...
                if tp_type == 'flat':
                    ...
                    atr = row['atr']
                    tp_price = entry_price - (atr * tp_value)
...
print("=" * 100)
print("TP OPTIMIZATION TEST (0.8 ATR limit offset fixed)")
print("=" * 100)
print()

# Test flat % TP
print("=" * 100)
print("FLAT % TAKE PROFIT (3% to 10%)")
print("=" * 100)

flat_results = []
for tp_pct in range(3, 11):
    r = test('flat', tp_pct)
    if r:
        flat_results.append(r)
        status = "✅" if r['profitable_months'] == 7 else f"({r['profitable_months']}/7)"
        print(f"\nTP = {tp_pct}% {status}")
        print(f"  R/DD: {r['return_dd']:5.2f}x | Ret: {r['return']:+7.1f}% | DD: {r['max_dd']:6.2f}%")
...
print("\n" + "=" * 100)
print("ATR-BASED TAKE PROFIT (3 ATR to 10 ATR)")
print("=" * 100)

atr_results = []
for tp_atr in range(3, 11):
    r = test('atr', tp_atr)
    if r:
        atr_results.append(r)
        status = "✅" if r['profitable_months'] == 7 else f"({r['profitable_months']}/7)"
        print(f"\nTP = {tp_atr} ATR {status}")
        print(f"  R/DD: {r['return_dd']:5.2f}x | Ret: {r['return']:+7.1f}% | DD: {r['max_dd']:6.2f}%")
        print(f"  Trades: {r['trades']:2d} | Win: {r['win_rate']:5.1f}% | TP Rate: {r['tp_rate']:5.1f}%")
        print(f"  Jun: ${r['jun']:+6.2f} | Jul: ${r['jul']:+6.2f} | Aug: ${r['aug']:+6.2f} | Sep: ${r['sep']:+6.2f}")
        print(f"  Oct: ${r['oct']:+6.2f} | Nov: ${r['nov']:+6.2f} | Dec: ${r['dec']:+7.2f}")
...
if flat_results:
    ...
    print(f"\n🏆 BEST FLAT %: TP = {best_flat['tp_value']:.0f}%")
...
if atr_results:
    ...
    print(f"\n🏆 BEST ATR: TP = {best_atr['tp_value']:.0f} ATR")
...
# Overall winner
all_results = flat_results + atr_results
...
print("=" * 100)
tp_label = f"{winner['tp_value']:.0f}%" if winner['tp_type'] == 'flat' else f"{winner['tp_value']:.0f} ATR"
print(f"TP = {tp_label} ({winner['tp_type']})")
