#!/usr/bin/env python3
"""MOODENG expanded test - 30 configs around sweet spot"""
import pandas as pd
import numpy as np
import time
...
print("\nTesting 30 configs: RSI 68-72 × offset 0.6/0.8/1.0 × TP 6-10%")
results = []
n = 0
for rsi in [68, 70, 72]:
    for offset in [0.6, 0.8, 1.0]:
        for tp in [6, 7, 8, 9, 10]:
            ...
            if r:
                results.append(r)
            if n % 10 == 0:
                print(f"  {n}/45 configs ({time.time()-start:.0f}s)...")

print(f"\nValid: {len(results)}/45")

if results:
    results.sort(key=lambda x: x['return_dd'], reverse=True)
    print(f"\n{'='*90}")
    print("MOODENG SHORT REVERSAL - TOP 5 CONFIGS")
    print(f"{'='*90}")
    ...
    w = results[0]
    print(f"\n{'='*90}")
    print(f"BEST: RSI>{w['rsi']} | {w['offset']:.1f}ATR | TP{w['tp']}%")
    print(f"{'='*90}")
