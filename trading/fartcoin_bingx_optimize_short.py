#!/usr/bin/env python3
"""
FARTCOIN BingX SHORT Strategy Optimization
Priority: Fix underfitting (only 3 trades) by relaxing constraints
"""
...
class ShortOptimizer:
    """Optimize SHORT strategy for BingX"""
    ...
    def check_entry(self, i, config):
        ...
        # Distance threshold (KEY PARAMETER TO OPTIMIZE)
        if row['sma_distance'] > -config['sma_distance_min']:
            return False
        ...
        # Explosive body (KEY PARAMETER TO OPTIMIZE)
        if row['body_pct'] < config['body_threshold']:
            return False

        # Volume spike (KEY PARAMETER TO OPTIMIZE)
        ...
        # RSI
        if row['rsi'] < config['rsi_min'] or row['rsi'] > config['rsi_max']:
            return False
    ...
    def optimize(self):
        ...
        print("="*80)
        print("FARTCOIN BINGX SHORT STRATEGY OPTIMIZATION")
        print("="*80)
        print("\nTesting systematic parameter variations...")

        # Parameter grid (PRIORITIZED based on baseline analysis)
        param_grid = {
            # PRIORITY 1: Distance threshold (current 2.0% eliminates 97.4% of candles)
            'sma_distance_min': [0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0, 2.5],

            # PRIORITY 2: Stop loss width (100% SL hit rate at 3x)
            'stop_atr_mult': [3.0, 3.5, 4.0, 4.5, 5.0, 6.0],

            # PRIORITY 3: Explosive body threshold (0.5% of candles at 1.2%)
            'body_threshold': [0.6, 0.8, 1.0, 1.2, 1.5],

            # PRIORITY 4: Volume multiplier (4.3% of candles at 3x)
            'volume_multiplier': [2.0, 2.5, 3.0, 3.5],

            # Keep these fixed (less critical)
            'target_atr_mult': [15.0],  # Keep high R:R target
            'wick_threshold': [0.35],
            'rsi_min': [25],
            'rsi_max': [55],
        }
        ...
        for i, config in enumerate(configs):
            ...
            if metrics is not None and metrics['total_trades'] >= 10:  # Require minimum 10 trades
                result = {**config, **metrics}
                self.results.append(result)

        print(f"\nCompleted {len(configs)} backtests")
        print(f"Found {len(self.results)} viable configurations (≥10 trades)\n")
    ...
    def print_top_configs(self, n=10):
        ...
        for i, (idx, row) in enumerate(df_sorted.head(n).iterrows(), 1):
            ...
            print(f"  Distance: {row['sma_distance_min']:.2f}% | Body: {row['body_threshold']:.2f}% | Vol: {row['volume_multiplier']:.1f}x")
        ...
        print(f"  Distance: {best_return['sma_distance_min']:.2f}% | Body: {best_return['body_threshold']:.2f}% | Vol: {best_return['volume_multiplier']:.1f}x")
...
if __name__ == "__main__":
    ...
    optimizer = ShortOptimizer(data_path)
    optimizer.load_data()
    optimizer.optimize()
