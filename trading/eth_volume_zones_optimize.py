#!/usr/bin/env python3
"""
ETH Volume Zones Optimization
Test parameters optimized for ETH's characteristics
"""
...
# ============================================================================
print("=" * 80)
print("ETH VOLUME ZONES OPTIMIZATION")
print("=" * 80)
print()

# Detect zones (use TRUMP's volume detection params)
...
sl_configs = [
    ...
    {'sl_type': 'fixed_pct', 'sl_value': 1.0},   # Wider stop
    {'sl_type': 'fixed_pct', 'sl_value': 1.5},   # Even wider
    {'sl_type': 'atr', 'sl_value': 1.0},         # ATR-based
    {'sl_type': 'atr', 'sl_value': 1.5},
]

tp_configs = [
    {'tp_type': 'rr_multiple', 'tp_value': 2.0},  # Tighter R:R
    {'tp_type': 'rr_multiple', 'tp_value': 3.0},  # Medium R:R
    {'tp_type': 'rr_multiple', 'tp_value': 4.0},  # TRUMP's config
    {'tp_type': 'fixed_pct', 'tp_value': 1.0},    # Fixed 1% TP
    {'tp_type': 'fixed_pct', 'tp_value': 1.5},    # Fixed 1.5% TP
]

session_filters = ['all', 'overnight', 'asia_eu', 'us']
direction_configs = [
    {'trade_longs': True, 'trade_shorts': False},   # LONGS only (shorts failed)
...
if len(results) > 0:
    ...
    # Compare to TRUMP
    print("=" * 80)
    print("COMPARISON TO TRUMP VOLUME ZONES")
    print("=" * 80)
    ...
    eth_return = f"{best['return']:+.2f}%"
    eth_dd = f"{best['max_dd']:.2f}%"
    eth_rdd = f"{best['return_dd']:.2f}x"
    eth_wr = f"{best['win_rate']:.1f}%"
    print(f"{'ETH':<10} {eth_return:>10} {eth_dd:>10} {eth_rdd:>12} {best['trades']:>8} {eth_wr:>10}")
    print()

    if best['return_dd'] > 8.0:
        print("🎯 EXCELLENT: ETH volume zones competitive with TRUMP!")
    elif best['return_dd'] > 5.0:
        print("✅ GOOD: ETH has solid risk-adjusted returns")
    elif best['return_dd'] > 3.0:
        print("⚠️  MODERATE: Tradeable but lower R/DD than TRUMP")
    else:
        print("❌ POOR: ETH volume zones underperform TRUMP")

    # Save best
    best['trades_df'].to_csv('results/ETH_volume_zones_optimized_trades.csv', index=False)
    print(f"\n✅ Best config trades saved to: results/ETH_volume_zones_optimized_trades.csv")
else:
