#!/usr/bin/env python3
"""
PEPE Volume Zones Optimization
Test volume zones approach on PEPE 1-minute data
"""
...
# ============================================================================
print("=" * 80)
print("PEPE VOLUME ZONES OPTIMIZATION")
print("=" * 80)
print()
...
if len(results) > 0:
    ...
    print(f"{'PENGU':<10} {'+17.39%':>10} {'-4.00%':>10} {'4.35x':>12} {100:>8} {'50.0%':>10}")
    print(f"{'ETH':<10} {'+3.78%':>10} {'-1.05%':>10} {'3.60x':>12} {17:>8} {'52.9%':>10}")
    pepe_return = f"{best['return']:+.2f}%"
    pepe_dd = f"{best['max_dd']:.2f}%"
    pepe_rdd = f"{best['return_dd']:.2f}x"
    pepe_wr = f"{best['win_rate']:.1f}%"
    print(f"{'PEPE':<10} {pepe_return:>10} {pepe_dd:>10} {pepe_rdd:>12} {best['trades']:>8} {pepe_wr:>10}")
    print()

    if best['return_dd'] > 8.0:
        print("🎯 EXCELLENT: PEPE volume zones competitive with TRUMP!")
    elif best['return_dd'] > 5.0:
        print("✅ GOOD: PEPE has solid risk-adjusted returns")
    elif best['return_dd'] > 3.0:
        print("⚠️  MODERATE: Tradeable but lower R/DD than TRUMP")
    else:
        print("❌ POOR: PEPE volume zones underperform")

    # Save best
    best['trades_df'].to_csv('results/PEPE_volume_zones_optimized_trades.csv', index=False)
    print(f"\n✅ Best config trades saved to: results/PEPE_volume_zones_optimized_trades.csv")
else:
