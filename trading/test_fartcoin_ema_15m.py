#!/usr/bin/env python3
"""
EMA 8/21 Crossover - 15m Timeframe Test
Broad sweep of key parameters
"""
...
# Load 15m data
df = pd.read_csv('/home/user/Carebiuro_windykacja/trading/fartcoin_6months_bingx_15m.csv')
df['timestamp'] = pd.to_datetime(df['timestamp'])
print(f"Loaded {len(df):,} 15m candles: {df['timestamp'].min()} to {df['timestamp'].max()}")
...
# Broad sweep - key combinations
print("\n" + "=" * 80)
print("EMA 8/21 CROSSOVER - 15M TIMEFRAME BROAD SWEEP")
print("=" * 80)
...
results = []
test_configs = [
    # 1h optimal params scaled for 15m
    (0.7, 7, 5),
    # Tighter params for faster TF
    (0.5, 4, 2),
    (0.5, 5, 3),
    (0.7, 5, 3),
    (0.7, 6, 4),
