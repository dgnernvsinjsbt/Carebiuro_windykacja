#!/usr/bin/env python3
"""Test different RSI trigger levels"""
import pandas as pd
import numpy as np
...
print("=" * 100)
print("RSI TRIGGER OPTIMIZATION (0.8 ATR offset, 10% TP fixed)")
print("=" * 100)
print()
...
if results:
    ...
    for i, r in enumerate(results, 1):
        ...
        print(f"#{i} RSI >{r['rsi']:2d} | R/DD: {r['return_dd']:6.2f}x | Ret: {r['return']:+7.1f}% | "
              f"Trades: {r['trades']:2d} | Win: {r['win_rate']:4.1f}% | Months: {r['profitable_months']}/7 {status}")

    winner = results[0]
    print("\n" + "=" * 100)
    print("🎯 BEST RSI TRIGGER")
    print("=" * 100)
    print(f"RSI > {winner['rsi']}")
