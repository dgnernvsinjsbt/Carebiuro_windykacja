#!/usr/bin/env python3
"""
EMA 8/21 Crossover - SL Optimization (TP fixed at 7 ATR)
Testing SL ATR levels: 1, 1.5, 2, 2.5, 3, 3.5, 4, 5
"""
...
TP_ATR = 7.0  # Fixed from TP optimization
MAX_WAIT_BARS = 20
...
def backtest(sl_atr):
    ...
    if not trades:
        return {'sl_atr': sl_atr, 'return': 0, 'max_dd': 0, 'rr': 0, 'trades': 0, 'win_rate': 0}
    ...
    return {
        'sl_atr': sl_atr, 'return': total_return, 'max_dd': max_dd,
        'rr': rr, 'trades': len(trades_df), 'win_rate': win_rate
    }

# Test SL levels
print("=" * 70)
print("EMA 8/21 CROSSOVER - SL OPTIMIZATION (TP 7 ATR)")
print("=" * 70)
print(f"Fixed: Limit 0.7 ATR | TP 7 ATR | Max wait 20 bars")
print("-" * 70)
print(f"{'SL ATR':>8} {'Return':>12} {'Max DD':>10} {'R:R':>10} {'Trades':>8} {'Win %':>8}")
print("-" * 70)
...
for sl in [1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0]:
    r = backtest(sl)
    results.append(r)
    print(f"{r['sl_atr']:>8} {r['return']:>+11.2f}% {r['max_dd']:>9.2f}% {r['rr']:>9.2f}x {r['trades']:>8} {r['win_rate']:>7.1f}%")

print("-" * 70)

# Best by R:R
best = max(results, key=lambda x: x['rr'])
print(f"\n🏆 BEST R:R: SL {best['sl_atr']} ATR → {best['rr']:.2f}x ({best['return']:+.2f}% return, {best['max_dd']:.2f}% DD)")

# Best by return
best_ret = max(results, key=lambda x: x['return'])
print(f"💰 BEST Return: SL {best_ret['sl_atr']} ATR → {best_ret['return']:+.2f}% ({best_ret['rr']:.2f}x R:R)")
