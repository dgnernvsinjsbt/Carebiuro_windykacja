#!/usr/bin/env python3
"""
FARTCOIN - DAILY RESET STRATEGY - GRID SEARCH

Test combinations of SL and TP around the sweet spot:
...
print("="*140)
print("FARTCOIN - DAILY RESET STRATEGY - GRID SEARCH OPTIMIZATION")
print("="*140)
print()
...
# Test grid
sl_values = [3.0, 3.5, 4.0, 4.5, 5.0]
tp_values = [6, 7, 8, 9, 10]
...
print("="*140)
print()
print(f"{'SL %':<8} {'TP %':<8} {'Return':<12} {'Max DD':<12} {'R/DD':<10} {'Trades':<10} {'WR %':<10} {'Expectancy':<12} {'Final $':<12}")
print("-"*140)

for _, row in results_df.iterrows():
    status = "🔥" if row['total_return'] > 20 else ("✅" if row['total_return'] > 0 else "❌")
    print(f"{row['sl_pct']:<8.1f} {row['tp_pct']:<8.0f} {row['total_return']:>+10.2f}%  {row['max_dd']:>10.2f}%  {row['return_dd']:>8.2f}x  {row['total_trades']:<10.0f} {row['win_rate']:>8.1f}%  {row['expectancy']:>+10.3f}%  ${row['final_equity']:>10.2f}  {status}")
...
for i, row in enumerate(top5_return.itertuples(), 1):
    print(f"{i}. SL={row.sl_pct:.1f}%, TP={row.tp_pct:.0f}% → Return: {row.total_return:+.2f}%, R/DD: {row.return_dd:.2f}x, WR: {row.win_rate:.1f}%, Trades: {row.total_trades:.0f}")

print()
print("🎯 TOP 5 BY RETURN/DD RATIO:")
print()
top5_rdd = results_df.nlargest(5, 'return_dd')
for i, row in enumerate(top5_rdd.itertuples(), 1):
    print(f"{i}. SL={row.sl_pct:.1f}%, TP={row.tp_pct:.0f}% → R/DD: {row.return_dd:.2f}x, Return: {row.total_return:+.2f}%, WR: {row.win_rate:.1f}%, Trades: {row.total_trades:.0f}")
...
if len(positive_returns) > 0:
    print(f"✅ {len(positive_returns)}/{len(results_df)} combinations are profitable")
    print()
    print(f"🏆 BEST OVERALL: SL={best_return['sl_pct']:.1f}%, TP={best_return['tp_pct']:.0f}%")
    print(f"   Return: {best_return['total_return']:+.2f}%")
    print(f"   R/DD: {best_return['return_dd']:.2f}x")
    ...
    print(f"   Expectancy: {best_return['expectancy']:+.3f}%")
else:
