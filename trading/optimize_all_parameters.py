"""
BROAD PARAMETER OPTIMIZATION
Wider ranges, less granular (~200 combinations)
"""
...
print("=" * 80)
print("BROAD PARAMETER OPTIMIZATION")
print("=" * 80)
...
# BROAD parameter grid
print("\nParameter grid (BROAD ranges):")
param_grid = {
    'rsi_low': [20, 25, 30],
    'rsi_high': [65, 70, 75],
    'limit_pct': [0.3, 1.0],
...
results_df.to_csv('melania_optimization_results.csv', index=False)
print(f"💾 Saved: melania_optimization_results.csv")

# Top 10
print("\n" + "=" * 80)
print("TOP 10:")
print("=" * 80)
print("\n| # | RSI   | Limit | SL  | TP  | Risk | Return   | DD    | R/DD   | Trades | Win% |")
print("|---|-------|-------|-----|-----|------|----------|-------|--------|--------|------|")

for i, (idx, row) in enumerate(results_df.head(10).iterrows(), 1):
    print(f"| {i} | {row['rsi_low']:.0f}/{row['rsi_high']:.0f} | {row['limit_pct']:.1f}% | "
          f"{row['sl_mult']:.1f}x | {row['tp_mult']:.1f}x | {row['risk_pct']:.0f}% | "
          f"{row['return']:+6.0f}% | {row['max_dd']:5.1f}% | {row['return_dd']:6.2f}x | "
          f"{row['trades']:3.0f} | {row['win_rate']:4.1f}% |")
...
print("🏆 WINNER:")
print("=" * 80)
print(f"\n  RSI: {best['rsi_low']:.0f}/{best['rsi_high']:.0f} | Limit: {best['limit_pct']:.1f}% | "
      f"SL: {best['sl_mult']:.1f}x | TP: {best['tp_mult']:.1f}x | Risk: {best['risk_pct']:.0f}%")
print(f"  Return: {best['return']:+.1f}% | DD: {best['max_dd']:.1f}% | R/DD: {best['return_dd']:.2f}x")
print(f"  Trades: {best['trades']:.0f} | Win: {best['win_rate']:.1f}% | TP: {best['tp_rate']:.1f}%")
print(f"\n  vs Baseline (12.40x): {best['return_dd'] - 12.40:+.2f}x ({(best['return_dd'] / 12.40 - 1) * 100:+.0f}%)")
print("=" * 80)
