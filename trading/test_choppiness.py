...
# Choppiness: count direction changes in last N bars
def calc_reversals(series, lookback):
    reversals = []
    for i in range(len(series)):
        if i < lookback:
            ...
            for j in range(i-lookback+1, i):
                if (series.iloc[j] > series.iloc[j-1]) != (series.iloc[j+1] > series.iloc[j]):
                    changes += 1
            reversals.append(changes)
    return reversals

df['reversals_10'] = calc_reversals(df['close'], 10)
df['reversals_20'] = calc_reversals(df['close'], 20)

def test(max_reversals, lookback):
    rsi_ob = 66
    ...
    reversal_col = f'reversals_{lookback}'
    ...
    for i in range(300, len(df)):
        ...
        if not position and not pending_order and i > 0:
            ...
            # CHOPPINESS FILTER
            if row[reversal_col] > max_reversals:
                continue
...
results = []
for max_rev, lookback in configs:
    r = test(max_rev, lookback)
    if r:
        results.append(r)
        status = "✅" if r['oct_dec_ok'] else "❌"
        print(f"\nMax{max_rev} reversals in {lookback}bars {status}")
...
if valid:
    ...
    best = valid[0]
    print("\n" + "=" * 80)
    print(f"BEST: Max {best['max_rev']} reversals in {best['lookback']} bars")
