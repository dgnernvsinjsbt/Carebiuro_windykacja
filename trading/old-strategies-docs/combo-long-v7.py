#!/usr/bin/env python3
"""
FARTCOIN/USDT Combination Approach V7 - LONG VERSION
Implements 3 key enhancements for LONG trades:
...
Goal: 4-6x Return/Drawdown ratio (vs SHORT's 2.45x)
"""
...
class ComboLongV7:
    """
    Enhanced LONG strategy combining:
    - Confirmation candle filtering
    - Resistance breakout timing
    ...
    def __init__(self, data_path: str, initial_capital: float = 10000):
        ...
        # Configuration - Optimized for 4-6x R:R target (LONG)
        self.config = {
            'body_threshold': 1.2,           # Min body % for entry
            'volume_multiplier': 3.0,        # Min volume surge
            'confirmation_vol_mult': 1.2,    # Confirmation candle volume (relaxed)
            'rsi_long_min': 40,              # Min RSI for long (slightly more lenient)
            'stop_atr_mult': 2.0,            # TIGHTER stop (2x ATR)
            'target_atr_mult_normal': 12.0,  # Normal target (6:1 R:R)
            'target_atr_mult_high_vol': 20.0, # High vol target (10:1 R:R)
            'base_risk_pct': 1.0,            # Conservative base risk
            'max_risk_pct': 3.0,             # Max risk
            'win_streak_scaling': 0.5,       # Moderate risk scaling on wins
            'use_trailing_stop': True,       # Enable trailing stops
            'use_partial_exits': True,       # Enable partial profit taking
            'max_hold_hours': 24,            # Standard hold time
            'resistance_proximity_pct': 4.0, # Within 4% of resistance (relaxed for more entries)
            'swing_high_period': 20,         # Swing high lookback
            'volatility_expansion': 1.2      # Vol expansion threshold
        }
    ...
    def load_data(self):
        ...
        print(f"\n{'='*70}")
        print(f"COMBO LONG V7 - Confirmation + Resistance Breakout + Dynamic TP")
        print(f"{'='*70}")
        print(f"Configuration:")
        print(f"  Body Threshold:     {self.config['body_threshold']:.1f}%")
        print(f"  Volume Multiplier:  {self.config['volume_multiplier']:.1f}x")
        print(f"  Confirmation Vol:   {self.config['confirmation_vol_mult']:.1f}x (relaxed)")
        print(f"  Stop Distance:      {self.config['stop_atr_mult']:.1f}x ATR (TIGHTER)")
        print(f"  Normal Target:      {self.config['target_atr_mult_normal']:.1f}x ATR (6:1 R:R)")
        print(f"  High Vol Target:    {self.config['target_atr_mult_high_vol']:.1f}x ATR (10:1 R:R)")
        print(f"  Resistance Prox:    {self.config['resistance_proximity_pct']:.1f}% (relaxed)")
        ...
        # Volatility metrics for dynamic targets
        ...
        self.df['swing_high'] = self.df['high'].rolling(
            window=self.config['swing_high_period'],
            center=True
        ).max()
    ...
    def detect_explosive_pattern(self, idx: int):
        """Detect initial Explosive Bullish Breakout pattern"""
        if idx < 200:
            return False
        ...
        if (row['is_bullish'] and
            ...
            row['rsi'] > cfg['rsi_long_min'] and row['rsi'] < 75):
            return True

        return False

    def check_confirmation_candle(self, idx: int):
        """Check if next candle confirms the breakout"""
        ...
        # 1. Also bullish
        is_bullish = curr_row['is_bullish']

        # 2. Continues upward
        continues_up = curr_row['close'] > prev_row['close']

        # 3. Volume still elevated
        vol_elevated = curr_row['vol_ratio'] >= self.config['confirmation_vol_mult']

        # All 3 must be met for LONG confirmation
        return is_bullish and continues_up and vol_elevated

    def check_resistance_proximity(self, idx: int):
        """Check if price is near resistance level (within 2%)"""
        row = self.df.loc[idx]

        # Get most recent swing high (resistance)
        resistance_level = None
        for i in range(idx - 1, max(0, idx - 100), -1):
            if not pd.isna(self.df.loc[i, 'swing_high']):
                resistance_level = self.df.loc[i, 'swing_high']
                break

        if resistance_level is None:
            return False

        # Check if current price is within proximity of resistance
        price_to_resistance_pct = abs((row['close'] - resistance_level) / resistance_level) * 100

        return price_to_resistance_pct <= self.config['resistance_proximity_pct']
    ...
    def backtest(self):
        ...
        for idx in range(200, len(self.df) - 1):
            row = self.df.loc[idx]
            ...
            if in_position and position:
                ...
                if exit_triggered:
                    ...
                    self.trades.append({
                        ...
                        'resistance_entry': position['resistance_entry']
                    })
                    ...
                    if trade_count <= 20 or trade_count % 10 == 0:
                        ...
                        res_marker = "NearRes" if position['resistance_entry'] else ""
                        print(f"Trade {trade_count}: {vol_marker:7s} {res_marker:8s} | "
            ...
            if not in_position:
                ...
                if pattern_detected_idx == idx - 1:
                    ...
                    if has_confirmation:
                        ...
                        # ENHANCEMENT 2: Resistance proximity check
                        near_resistance = self.check_resistance_proximity(idx)
                        ...
                        if near_resistance:
                            ...
                            # ENHANCEMENT 2: Tighter stop (2x ATR)
                            ...
                            if position_size > 0:
                                position = {
                                    ...
                                    'direction': 'long',
                                    'pattern': 'Confirmed Explosive Bullish + Resistance',
                                    'stop_loss': stop_loss,
                                    ...
                                    'resistance_entry': near_resistance
                                }
                                in_position = True
        ...
        print(f"✓ Resistance Entries: {resistance_entries}")
    ...
    def analyze_results(self):
        ...
        # Save to CSV
        df_trades.to_csv('/workspaces/Carebiuro_windykacja/strategies/combo-long-trades.csv', index=False)
        df_equity.to_csv('/workspaces/Carebiuro_windykacja/strategies/combo-long-equity.csv', index=False)
        print("✓ Saved: combo-long-trades.csv")
        print("✓ Saved: combo-long-equity.csv\n")
        ...
        if rr_ratio >= 4.0:
            print(f"   ✅ TARGET MET! (Goal: 4-6x)")
        else:
            print(f"   Target: 4-6x (Current: {rr_ratio:.2f}x)")
        print(f"")
        print(f"📈 TRADES")
        print(f"{'-'*70}")
        ...
        print(f"\n📊 COMPARISON TO SHORT BASELINE (Combo V7)")
        print(f"{'-'*70}")
        print(f"                    SHORT       →    LONG        →    Change")
        print(f"{'-'*70}")
        baseline_return = 8.71
        baseline_dd = -3.55
        baseline_rr = 2.45
        ...
        criteria = [
            ("R:R ratio: 3-6x", rr_ratio >= 3.0 and rr_ratio <= 6.0, f"{rr_ratio:.2f}x"),
            ("Return: +10-18%", total_return_pct >= 10.0 and total_return_pct <= 18.0, f"{total_return_pct:+.2f}%"),
            ...
            ("Trades: 10-16", total_trades >= 10 and total_trades <= 16, f"{total_trades}")
        ]
