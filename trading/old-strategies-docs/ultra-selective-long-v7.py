#!/usr/bin/env python3
"""
FARTCOIN/USDT Ultra-Selective LONG Strategy V7
Goal: Achieve 6-12x Return/Drawdown ratio through extreme selectivity (LONG trades only)
Target: 5-10 trades only, 50-60% win rate, minimal drawdown
Inverted version of the SHORT strategy to capture explosive bullish breakouts
"""
...
class UltraSelectiveLongV7:
    """
    Ultra-selective LONG strategy with extreme filters:
    - Only 5-10 highest quality trades
    - Massive bullish moves only (body > 1.5%)
    - Institutional volume (3.5x average, top 10%)
    - Strong uptrend (above 50 & 200 SMA)
    - Multi-indicator confluence (RSI > 52, expanding ATR)
    - Clean bullish candle structure
    - Wide stop (3.5x ATR) + very wide target (25x ATR) = 7.1:1 R:R
    """
    ...
    def __init__(self, data_path: str, initial_capital: float = 10000):
        ...
        self.config = {
            ...
            'close_position_pct': 65,     # Close in top 35% of range (100 - 35 = 65)
            'rsi_min': 52,                # Bullish bias (inverted from rsi_max: 48)
    ...
    def load_data(self):
        ...
        print(f"\n{'='*70}")
        print(f"ULTRA-SELECTIVE LONG STRATEGY V7")
        print(f"{'='*70}")
        print(f"Configuration:")
        print(f"  Body Threshold:        {self.config['body_threshold']:.1f}% (MASSIVE BULLISH)")
        ...
        print(f"  Stop Distance:         {self.config['stop_atr_mult']:.1f}x ATR (WIDE)")
        print(f"  Target Distance:       {self.config['target_atr_mult']:.1f}x ATR (VERY WIDE)")
        ...
        print(f"  Swing High Resistance: {'YES' if self.config['use_swing_high_filter'] else 'NO'}")
        print(f"{'='*70}\n")
        ...
        # Volume percentile (top 10% of ALL candles)
        ...
        # Swing highs (resistance levels) - optional, only if enabled
        if self.config.get('use_swing_high_filter', False):
            swing_window = 20
            self.df['swing_high'] = self.df['high'].rolling(
                window=swing_window,
                center=True
            ).max()
        ...
        self.df['above_50'] = self.df['close'] > self.df['sma_50']
        self.df['above_200'] = self.df['close'] > self.df['sma_200']
        self.df['strong_uptrend'] = self.df['above_50'] & self.df['above_200']
    ...
    def detect_pattern(self, idx: int):
        """
        Detect ultra-selective bullish breakout pattern
        Very strict multi-criteria filter (inverted for LONGS)
        """
        if idx < 200:
            return None
        ...
        # CRITERION 1: Large bullish move (inverted)
        ...
        # CRITERION 4: Multi-indicator confluence (inverted for uptrend)
        if not row['strong_uptrend']:
            return None
        if row['rsi'] <= cfg['rsi_min'] or row['rsi'] > 75:
            return None
        if not row['atr_expanding']:
            return None

        # CRITERION 5: Clean candle structure (inverted for bullish)
        ...
        # OPTIONAL: Swing high filter
        if cfg['use_swing_high_filter']:
            if pd.isna(row['swing_high']):
                return None
            # Not used for now - distance from swing high check removed

        # OPTIONAL: Confirmation candle
        entry_idx = idx
        if cfg['use_confirmation']:
            next_row = self.df.loc[idx + 1]
            if not next_row['is_bullish']:
                return None
        ...
        return {
            'direction': 'long',
            'pattern': 'Ultra-Selective Bullish Breakout',
            'entry_idx': entry_idx,
    ...
    def backtest(self):
        ...
        print(f"Running Ultra-Selective LONG Backtest...\n")
        print(f"Scanning {len(self.df):,} candles for perfect bullish setups...\n")
        ...
        for idx in range(200, len(self.df)):
            ...
            if in_position and position:
                ...
                # Calculate P&L and R-multiple (inverted for LONG)
                ...
                # Trailing stops at 3R, 5R, 8R (inverted for LONG)
                if r_multiple >= 3.0:
                ...
                # Stop loss (inverted for LONG)
                if current_price <= position['stop_loss']:
                    exit_triggered, exit_reason = True, 'Stop Loss'
                    exit_price = position['stop_loss']

                # Take profit (inverted for LONG)
                ...
                if exit_triggered:
                    # Calculate final P&L (inverted for LONG)
            ...
            if not in_position:
                ...
                if signal:
                    ...
                    # Inverted for LONG: stop below, target above
    ...
    def analyze_results(self):
        """Analyze results and compare to SHORT baseline"""
        print(f"{'='*70}")
        print("RESULTS")
        print(f"{'='*70}\n")
        ...
        # Save CSVs
        trades_csv = '/workspaces/Carebiuro_windykacja/strategies/ultra-selective-long-trades.csv'
        equity_csv = '/workspaces/Carebiuro_windykacja/strategies/ultra-selective-long-equity.csv'
        ...
        print(f"Win Rate:        {win_rate:.1f}% {'✅' if 50 <= win_rate <= 65 else '⚠️'}")
        ...
        # Comparison to SHORT V7 baseline
        print(f"\n📊 COMPARISON TO SHORT V7 BASELINE")
        print(f"{'-'*70}")
        baseline = {
            'return': 7.88,
        ...
        print(f"                    LONG V7      |   SHORT V7     |  Difference")
        print(f"{'-'*70}")
        ...
        print(f"\n✅ SUCCESS CRITERIA")
        print(f"{'-'*70}")
        print(f"R:R ratio 5-12x:    {'✅ YES' if 5.0 <= rr_ratio <= 12.0 else '❌ NO':20s} (Actual: {rr_ratio:.2f}x)")
        print(f"Return +8-15%:      {'✅ YES' if 8.0 <= total_return_pct <= 15.0 else '❌ NO':20s} (Actual: {total_return_pct:+.2f}%)")
        print(f"Max DD -1 to -3%:   {'✅ YES' if -3.0 <= max_dd <= -1.0 else '❌ NO':20s} (Actual: {max_dd:.2f}%)")
        print(f"Win rate 50-65%:    {'✅ YES' if 50.0 <= win_rate <= 65.0 else '❌ NO':20s} (Actual: {win_rate:.1f}%)")
...
if __name__ == "__main__":
    ...
    if results:
        ...
        short_baseline_rr = 6.01
        if results['rr_ratio'] >= short_baseline_rr:
            print(f"\n🎯 LONGS OUTPERFORM: {results['rr_ratio']:.2f}x vs SHORT's {short_baseline_rr:.2f}x")
        else:
            print(f"\n📉 LONGS UNDERPERFORM: {results['rr_ratio']:.2f}x vs SHORT's {short_baseline_rr:.2f}x")

        if 5.0 <= results['rr_ratio'] <= 12.0:
            print(f"   ✅ TARGET ACHIEVED: {results['rr_ratio']:.2f}x R:R ratio (5-12x target)")
        elif results['rr_ratio'] > 12.0:
            print(f"   🚀 EXCEPTIONAL: {results['rr_ratio']:.2f}x R:R ratio (exceeded 12x target!)")
        else:
            print(f"   ⚠️  BELOW TARGET: {results['rr_ratio']:.2f}x R:R ratio (target: 5-12x)")
        print(f"{'='*70}\n")
