#!/usr/bin/env python3
"""Test different stop loss ATR values"""
import pandas as pd
import numpy as np
...
def backtest_sl(sl_atr):
    """Test with specific stop loss"""
    # Fixed params
    rsi_ob = 65
    limit_offset_atr = 0.1  # OPTIMAL from previous test
    tp_atr = 3.0
    min_move = 0.8
    min_momentum = 0
        ...
        if not position and not pending_order and i > 0:
            ...
            if prev_row['rsi'] > rsi_ob and row['rsi'] <= rsi_ob:
                if row['avg_move_size'] >= min_move:
                    ...
                    sl_price = limit_price + (atr * sl_atr)  # VARIABLE
...
print("=" * 70)
print("TESTING STOP LOSS (1.0 to 3.0 ATR)")
print("Fixed: Offset=0.1 ATR, TP=3.0 ATR, RSI=65, Move=0.8%, Momentum=0%")
print("=" * 70)

sl_values = [1.0, 1.2, 1.5, 2.0, 2.5, 3.0]

results = []
for sl in sl_values:
    result = backtest_sl(sl)
    if result:
...
if results:
    ...
    print(f"\n{'SL ATR':<8} {'Return':<10} {'Max DD':<10} {'R/DD':<8} {'Trades':<8} {'Win%':<8}")
    print("-" * 70)

    for r in results:
        print(f"{r['sl_atr']:<8.1f} {r['return']:>+9.1f}% {r['max_dd']:>9.2f}% {r['return_dd']:>7.2f}x {r['trades']:>7d} {r['win_rate']:>7.1f}%")
