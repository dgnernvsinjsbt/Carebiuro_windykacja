#!/usr/bin/env python3
"""Quick PEPE test with last 7 days of data"""
import pandas as pd
import numpy as np
...
def run_bb3_backtest(df: pd.DataFrame, bb_std: float, atr_sl: float, atr_tp: float) -> dict:
    ...
    win_pnls = [t['pnl'] * 100 for t in trades if t['pnl'] > 0]
    loss_pnls = [t['pnl'] * 100 for t in trades if t['pnl'] < 0]
...
# Use last 7 days only for quick test
cutoff = df['timestamp'].max() - timedelta(days=7)
df = df[df['timestamp'] >= cutoff].reset_index(drop=True)

print(f"Using {len(df):,} candles ({df['timestamp'].min()} to {df['timestamp'].max()})")
print()
...
for name, bb_std, atr_sl, atr_tp in strategies:
    ...
    if result:
        ...
        print(f"{name} Strategy (BB{bb_std}, ATR SL/TP: {atr_sl}/{atr_tp}):")
...
if results:
    ...
    print("="*60)
    print(f"BEST STRATEGY: {best['strategy']} with R:R Ratio {best['rr_ratio']:.2f}x")
    print(f"Net P&L: {best['net_pnl']:+.2f}%, Max DD: -{best['max_dd']:.2f}%")
    print("="*60)
