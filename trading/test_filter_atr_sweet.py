#!/usr/bin/env python3
"""Filter Test: ATR 0.8-2.5% (sweet spot)"""
import pandas as pd
import numpy as np
...
if trades:
    ...
    print(f"ATR 0.8-2.5%: Return {total_return:+.2f}%, DD {max_dd:.2f}%, R/DD {total_return/max_dd if max_dd > 0 else 0:.2f}x, Trades {len(trades_df)}, WR {win_rate:.1f}%, Skipped {skipped}")
