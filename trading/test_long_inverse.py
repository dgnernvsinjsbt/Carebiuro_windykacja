#!/usr/bin/env python3
"""Test LONG inverse strategy: RSI low → swing high break → limit below → long retest"""
import pandas as pd
import numpy as np
...
def find_swing_low(df, start_idx, end_idx):
    """Find swing low between two indices (for SL)"""
...
def test(rsi_trigger, limit_atr_offset, tp_pct):
    ...
    for i in range(lookback, len(df)):
        ...
        # RSI drops below trigger (oversold)
        ...
        # Wait for break above swing high
        if armed and swing_high is not None and not limit_pending:
            if row['high'] > swing_high:
                # BREAK! Place limit order BELOW break line
                atr = row['atr']
                limit_price = swing_high - (atr * limit_atr_offset)
...
print("=" * 110)
print("LONG INVERSE STRATEGY TEST")
print("Logic: RSI <X → swing high break → limit below → LONG retest")
print("=" * 110)
print()

# Test grid: RSI 28/30/32 × Offset 0.4/0.6/0.8/1.0 × TP 5/6/7/8
results = []
total_configs = 0
...
if results:
    ...
    # Compare to SHORT strategy
    print("\n" + "=" * 110)
    print("📊 LONG vs SHORT COMPARISON")
    print("=" * 110)
    print(f"LONG  (RSI <{winner['rsi']}):  R/DD {winner['return_dd']:6.2f}x | Return {winner['return']:+7.1f}% | Trades {winner['trades']}")
    print(f"SHORT (RSI >72):  R/DD  53.96x | Return +1330.4% | Trades 45")
    print()

    if winner['return_dd'] > 10:
        print("✅ LONG strategy is VIABLE!")
    else:
        print("❌ LONG strategy underperforms SHORT")

    # Check if any config has all 7 months profitable
    all_green = [r for r in results if r['profitable_months'] == 7]
    if all_green:
        print(f"\n✅ Found {len(all_green)} configs with ALL 7 months profitable!")
        best_green = all_green[0]
        print(f"   Best: RSI <{best_green['rsi']} | Offset {best_green['offset']:.1f} ATR | TP {best_green['tp']}%")
        print(f"   R/DD: {best_green['return_dd']:.2f}x | Return: {best_green['return']:+.1f}%")
    else:
