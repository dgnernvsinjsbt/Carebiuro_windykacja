...
# Load cached XRP data
df = pd.read_csv('xrp_usdt_1m_lbank.csv')
df['timestamp'] = pd.to_datetime(df['timestamp'])

print(f"\n{'='*60}")
print(f"TESTING XRP/USDT")
print(f"{'='*60}")
...
def run_bb3_backtest(df: pd.DataFrame, bb_std: float, atr_sl: float, atr_tp: float) -> dict:
    ...
    avg_win = sum(wins_list) / len(wins_list) * 100 if wins_list else 0
    avg_loss = sum(losses_list) / len(losses_list) * 100 if losses_list else 0
...
if results:
    ...
    print(f"{'='*60}")
    print(f"Risk:Reward Ratio: {best['rr_ratio']:.2f}x")
    ...
    print("\nNo trades executed for any strategy")
