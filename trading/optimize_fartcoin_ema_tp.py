#!/usr/bin/env python3
"""
EMA 8/21 Crossover - TP Optimization
Testing TP ATR levels: 4, 6, 8, 10, 12, 14, 16
"""
...
def backtest(tp_atr):
    ...
    if not trades:
        return {'tp_atr': tp_atr, 'return': 0, 'max_dd': 0, 'rr': 0, 'trades': 0, 'win_rate': 0}
    ...
    return {
        'tp_atr': tp_atr, 'return': total_return, 'max_dd': max_dd,
        'rr': rr, 'trades': len(trades_df), 'win_rate': win_rate
    }

# Test TP levels
print("=" * 70)
print("EMA 8/21 CROSSOVER - TP OPTIMIZATION")
print("=" * 70)
print(f"Fixed: Limit 0.7 ATR | SL 2 ATR | Max wait 20 bars")
print("-" * 70)
print(f"{'TP ATR':>8} {'Return':>12} {'Max DD':>10} {'R:R':>10} {'Trades':>8} {'Win %':>8}")
print("-" * 70)

results = []
for tp in [4, 5, 6, 7, 8, 9, 10, 12, 14, 16]:
    r = backtest(tp)
    results.append(r)
    print(f"{r['tp_atr']:>8} {r['return']:>+11.2f}% {r['max_dd']:>9.2f}% {r['rr']:>9.2f}x {r['trades']:>8} {r['win_rate']:>7.1f}%")

print("-" * 70)

# Best by R:R
best = max(results, key=lambda x: x['rr'])
print(f"\n🏆 BEST R:R: TP {best['tp_atr']} ATR → {best['rr']:.2f}x ({best['return']:+.2f}% return, {best['max_dd']:.2f}% DD)")

# Best by return
best_ret = max(results, key=lambda x: x['return'])
print(f"💰 BEST Return: TP {best_ret['tp_atr']} ATR → {best_ret['return']:+.2f}% ({best_ret['rr']:.2f}x R:R)")
