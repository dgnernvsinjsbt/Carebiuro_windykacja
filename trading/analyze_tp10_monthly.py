#!/usr/bin/env python3
"""Detailed month-by-month analysis for TP 10% config"""
import pandas as pd
import numpy as np
...
print("=" * 100)
print("MONTH-BY-MONTH BREAKDOWN (0.8 ATR LIMIT + 10% TP)")
print("=" * 100)
print()
...
for month, name in zip(months, month_names):
    ...
    losers = month_trades[month_trades['pnl_dollar'] < 0]
    win_rate = (len(winners) / len(month_trades)) * 100
    tp_rate = (len(month_trades[month_trades['exit_reason'] == 'TP']) / len(month_trades)) * 100
...
# Print table
print(f"{'Month':<6} {'Trades':>7} {'Win%':>7} {'TP%':>7} {'P&L':>11} {'MaxDD':>10} {'R:DD':>8} {'Avg W':>9} {'Avg L':>9}")
print("-" * 100)

for r in results:
    status = "✅" if r['pnl'] > 0 else "❌" if r['pnl'] < 0 else "⚪"
    print(f"{r['month']:<6} {r['trades']:>7} {r['win_rate']:>6.1f}% {r['tp_rate']:>6.1f}% "
          f"${r['pnl']:>+10.2f} ${r['max_dd']:>9.2f} {r['return_dd']:>7.2f}x "
          f"${r['avg_winner']:>+8.2f} ${r['avg_loser']:>+8.2f} {status}")
...
overall_tp_rate = (len(trades_df[trades_df['exit_reason'] == 'TP']) / total_trades) * 100

all_winners = trades_df[trades_df['pnl_dollar'] > 0]
all_losers = trades_df[trades_df['pnl_dollar'] < 0]
overall_avg_winner = all_winners['pnl_dollar'].mean()
overall_avg_loser = all_losers['pnl_dollar'].mean()
...
print(f"{'TOTAL':<6} {total_trades:>7} {overall_win_rate:>6.1f}% {overall_tp_rate:>6.1f}% "
      f"${total_pnl:>+10.2f} {overall_max_dd:>9.2f}% {overall_rdd:>7.2f}x "
      f"${overall_avg_winner:>+8.2f} ${overall_avg_loser:>+8.2f}")
print()
...
if losing_months:
    ...
    for r in sorted(losing_months, key=lambda x: x['pnl']):
        print(f"  {r['month']}: ${r['pnl']:+.2f} ({r['trades']} trades, {r['win_rate']:.1f}% win, {r['tp_rate']:.1f}% TP)")
    print()
...
print(f"Avg Winner: ${overall_avg_winner:.2f} | Avg Loser: ${overall_avg_loser:.2f}")
print(f"Profit Factor: {abs(overall_avg_winner / overall_avg_loser):.2f}x" if overall_avg_loser != 0 else "N/A")
print(f"Total Return: +{total_return:.1f}% | Final Equity: ${equity_curve[-1]:.2f}")
