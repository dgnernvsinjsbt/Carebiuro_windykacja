"""
Test RANK #1 with 100% EQUITY per trade (all-in, max aggression)
0.3%, 1.5x SL, 2.5x TP
"""
...
print("=" * 80)
print("RANK #1 - 100% EQUITY PER TRADE (ALL-IN)")
print("=" * 80)
...
print(f"  Position Size: 100% OF EQUITY (ALL-IN)")
...
print(f"\n  Avg Win: {df_trades[df_trades['pnl_dollars'] > 0]['price_change_pct'].mean():.2f}%")
print(f"  Avg Loss: {df_trades[df_trades['pnl_dollars'] < 0]['price_change_pct'].mean():.2f}%")
...
print("\n| Month    | Trades | Win% | Monthly Return | Equity      |")
print("|----------|--------|------|----------------|-------------|")
...
for month in sorted(df_trades['month'].unique()):
    ...
    print(f"| {month} | {total:6d} | {win_pct:4.0f}% | {monthly_return:+12.2f}% | ${equity_end:10,.2f} |")
    
    equity_start = equity_end

df_trades.to_csv('melania_100pct_trades.csv', index=False)
print(f"\n💾 Saved trades to: melania_100pct_trades.csv")

print("\n" + "=" * 80)
print("🚀 100% EQUITY - MAXIMUM AGGRESSION MODE")
print("=" * 80)
print(f"Starting with $100 → Ending with ${equity:,.2f}")
print("=" * 80)
