#!/usr/bin/env python3
"""
POPRAWIONE POSITION SIZING:
- Dla 10 wejść: każde MAX 10% equity
- Total exposure <= 100% equity
- Risk 5% equity total, ale z limitem na wielkość pozycji
"""
import pandas as pd
import numpy as np
...
print("="*140)
print("STRATEGIA: DOWNTREND STRUCTURE + SCALING - POPRAWIONE POSITION SIZING")
print("="*140)
print()
                ...
                for level in scale_levels:
                    if not level['filled'] and row['high'] >= level['price']:
                        ...
                        if sl_dist_pct > 0 and sl_dist_pct <= 10.0:
                            ...
                            # 1. Oblicz ile chcemy risknąć na tym entry (5% equity / 10 entries = 0.5%)
                            target_risk = equity * 5.0 / max_scales / 100

                            # 2. Oblicz size potrzebny do tego riska
                            calculated_size = target_risk / (sl_dist_pct / 100)

                            # 3. CAP na max 10% equity (dla 10 entries = 100% total exposure max)
                            max_size_per_entry = equity / max_scales

                            # 4. Użyj mniejszego z dwóch
