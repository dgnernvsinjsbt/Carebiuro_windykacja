#!/usr/bin/env python3
"""Test different limit offset values"""
import pandas as pd
import numpy as np
...
def backtest_offset(limit_offset_atr):
    """Test with specific limit offset"""
    # Fixed params
    rsi_ob = 65
    sl_atr = 1.2
    tp_atr = 3.0
    min_move = 0.8
    min_momentum = 0  # No momentum filter for baseline
...
print("=" * 70)
print("TESTING LIMIT OFFSET (0.1 to 1.5 ATR)")
print("Fixed: SL=1.2 ATR, TP=3.0 ATR, RSI=65, Move=0.8%, Momentum=0%")
print("=" * 70)
...
for offset in offsets:
    result = backtest_offset(offset)
    if result:
        results.append(result)
        print(f"\nOffset {offset:.1f} ATR:")
...
if results:
    ...
    print(f"\n{'Offset':<8} {'Return':<10} {'Max DD':<10} {'R/DD':<8} {'Trades':<8} {'Win%':<8}")
    print("-" * 70)

    for r in results:
        print(f"{r['offset']:<8.1f} {r['return']:>+9.1f}% {r['max_dd']:>9.2f}% {r['return_dd']:>7.2f}x {r['trades']:>7d} {r['win_rate']:>7.1f}%")
