# Master Trader: XLM/USDT 1-Minute Strategy Discovery

## Objective
Become a master trader for XLM/USDT. Find a profitable trading strategy with minimum 2:1 R:R ratio (net P&L vs max drawdown).

## Data
- File: `xlm_usdt_1m_lbank.csv` (~43,000 candles, 30 days of 1m data)
...
### 1. Trading Sessions
Test each session separately to find XLM's best trading hours:
- **Asian**: 0-8 UTC
- **European**: 8-14 UTC
- **US**: 14-22 UTC
...
## Output Requirements
1. Create backtest script: `xlm_master_backtest.py`
2. Save results to: `results/xlm_master_results.csv`
...
Remember: XLM had poor results with BB3 strategy (-0.44% loss). The coin needs a DIFFERENT approach.
