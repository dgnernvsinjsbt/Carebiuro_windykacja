#!/usr/bin/env python3
"""Filter Test: RSI > 60 before drop"""
import pandas as pd
import numpy as np
...
for i in range(50, len(df_dec)):
    ...
    if not in_position and not tp_hit_today:
        ...
        if row['low'] <= trigger_price:
            # FILTER: RSI > 60
            if pd.notna(row['rsi']) and row['rsi'] <= 60:
                skipped += 1
                continue
...
if trades:
    ...
    print(f"RSI > 60: Return {total_return:+.2f}%, DD {max_dd:.2f}%, R/DD {total_return/max_dd if max_dd > 0 else 0:.2f}x, Trades {len(trades_df)}, WR {win_rate:.1f}%, Skipped {skipped}")
