#!/usr/bin/env python3
"""
TRUMP Volume Zones - Find Best Risk-Adjusted Return
Sort by Return/DD ratio (higher is better)
"""
...
print(f"Zones detected: {len(baseline_zones)}")
...
for sl_cfg, tp_cfg, session, dir_cfg, max_hold in product(
    ...
    # CRITICAL: Skip configs with max_dd = 0 (impossible)
    if max_dd >= -0.01:  # DD must be at least 0.01%
        continue
...
# SORT BY RETURN/DD RATIO (HIGHEST FIRST)
results = sorted(results, key=lambda x: x['return_dd_ratio'], reverse=True)

print(f"Generated {len(results)} valid configurations")
print()
print("=" * 120)
print("TOP 20 CONFIGURATIONS BY RETURN/DD RATIO (RISK-ADJUSTED)")
print("=" * 120)
print(f"{'Rank':<5} {'R/DD':<7} {'Return':<8} {'MaxDD':<8} {'SL Type':<12} {'SL':<6} {'TP Type':<12} {'TP':<6} {'Session':<10} {'Dir':<6} {'Hold':<5} {'Trades':<7} {'WR':<6}")
print("-" * 120)
...
for i, r in enumerate(results[:20], 1):
    ...
    print(f"{i:<5} {r['return_dd_ratio']:<7.2f} {r['return']:<7.2f}% {r['max_dd']:<7.2f}% "
          f"{cfg['sl_type']:<12} {cfg['sl_value']:<6.2f} {cfg['tp_type']:<12} {cfg['tp_value']:<6.1f} "
          f"{session_str:<10} {direction:<6} {cfg['max_hold_bars']:<5} {r['trades']:<7} {r['win_rate']:<5.1f}%")

# Best config by R/DD
best = results[0]
print("\n" + "=" * 80)
print("BEST RISK-ADJUSTED CONFIGURATION")
print("=" * 80)
print(f"Return/DD Ratio: {best['return_dd_ratio']:.2f}x ← HIGHEST")
...
# Save best
best['trades_df'].to_csv('results/TRUMP_volume_zones_best_riskadj_trades.csv', index=False)
print(f"\n✅ Best risk-adjusted trades saved to: results/TRUMP_volume_zones_best_riskadj_trades.csv")

print("\n" + "=" * 80)
print("COMPARISON")
print("=" * 80)
print(f"Baseline (5:1 R:R):       +9.89% | -1.54% DD | 6.42x R/DD")
print(f"Max Return Config:        +16.75% | -3.10% DD | 5.40x R/DD")
print(f"Best Risk-Adjusted:       {best['return']:+.2f}% | {best['max_dd']:.2f}% DD | {best['return_dd_ratio']:.2f}x R/DD ← BEST")
