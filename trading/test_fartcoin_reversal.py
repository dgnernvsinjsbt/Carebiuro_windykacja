#!/usr/bin/env python3
"""Test SHORT reversal strategy on FARTCOIN"""
import pandas as pd
import numpy as np
...
print("FARTCOIN SHORT REVERSAL TEST")
results = []
...
if results:
    ...
    for i, r in enumerate(results[:5], 1):
        print(f"#{i} RSI>{r['rsi']} {r['offset']:.1f}ATR {r['tp']}% | R/DD:{r['return_dd']:.2f}x Ret:{r['return']:+.1f}% {r['trades']}tr {r['win_rate']:.1f}% {r['profitable_months']}/7mo")

    w = results[0]
    print(f"\nBEST: RSI>{w['rsi']} {w['offset']:.1f}ATR {w['tp']}% | {w['return_dd']:.2f}x {w['return']:+.1f}% {w['trades']}tr")
