"""
Simplified monthly table - fewer columns
"""
import pandas as pd
import numpy as np
...
print('\n' + '=' * 140)
print('MONTHLY PERFORMANCE - SIMPLIFIED')
print('Config: +50%/-50% Dynamic Sizing | 2% Floor | 0.8% Surgical Filter')
print('=' * 140)

print(f"\n{'Month':<10} {'Trades':<7} {'L/S':<7} {'W/L':<7} {'WR%':<6} {'MaxW':<5} {'MaxL':<5} "
      f"{'AvgR%':<7} {'Return':<9} {'MaxDD':<9} {'R/DD':<7} {'Start$':<10} {'End$':<11} {'Cum%':<10}")
print('-' * 140)
...
for r in results:
    ...
    print(f"{r['month']:<10} {r['trades']:<7} {ls_str:<7} {wl_str:<7} "
          f"{r['win_rate']:>5.1f} {r['max_win_streak']:>4} {r['max_loss_streak']:>4} "
          f"{r['avg_risk']:>6.1f} "
...
print(f"{'TOTAL':<10} {total_trades:<7} {'':<7} {f'{total_winners}/{total_losers}':<7} "
      f"{(total_winners/total_trades*100):>5.1f} {'':<5} {'':<5} "
      f"{'':<7} "
...
print('LEGEND')
print('=' * 140)
print('L/S = Long/Short | W/L = Winners/Losers | WR% = Win Rate | MaxW = Max Win Streak | MaxL = Max Loss Streak')
print('AvgR% = Avg Risk % | Return = Monthly Return % | MaxDD = Max Drawdown % | R/DD = Return/DD Ratio')
print('Start$ = Starting Equity | End$ = Ending Equity | Cum% = Cumulative Return %')
print('=' * 140)

print('\n🏆 FINAL: $%.2f (+%.1f%%) | Max DD: %.2f%% | R/DD: %.2fx' %
      (final_equity, final_return, overall_max_dd, overall_rdd))
print('=' * 140)
