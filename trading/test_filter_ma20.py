#!/usr/bin/env python3
"""Filter Test: Price < MA20 (downtrend)"""
import pandas as pd
import numpy as np
...
for i in range(50, len(df_dec)):
    ...
    if not in_position and not tp_hit_today:
        ...
        if row['low'] <= trigger_price:
            # FILTER: Price < MA20
            if pd.notna(row['ma_20']) and trigger_price >= row['ma_20']:
                skipped += 1
                continue
...
if trades:
    ...
    print(f"Price < MA20: Return {total_return:+.2f}%, DD {max_dd:.2f}%, R/DD {total_return/max_dd if max_dd > 0 else 0:.2f}x, Trades {len(trades_df)}, WR {win_rate:.1f}%, Skipped {skipped}")
