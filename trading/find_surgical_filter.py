"""
Find a SURGICAL filter that removes worst 10-20 losers WITHOUT touching winners
Goal: Maximize worst loser removal while minimizing winner removal
"""
import pandas as pd
import numpy as np
...
print('=' * 140)
print('FINDING SURGICAL FILTER: Remove worst losers WITHOUT touching winners')
print('=' * 140)
...
# Test various surgical filters
filters = [
    ('SHORT + ATR < 0.8%',
     lambda t: (t['direction'] == 'SHORT') & (t['entry_atr_pct'] < 0.8)),

    ('SHORT + Move < 1.0%',
     lambda t: (t['direction'] == 'SHORT') & (t['entry_move_size'] < 1.0)),

    ('Signal Quality < -0.5%',
     lambda t: t['signal_quality'] < -0.5),

    ('SHORT + ATR < 1.0% + Move < 1.2%',
     lambda t: (t['direction'] == 'SHORT') & (t['entry_atr_pct'] < 1.0) & (t['entry_move_size'] < 1.2)),

    ('Consecutive losses >= 3',
     lambda t: t['consecutive_losses'] >= 3),

    ('Consecutive losses >= 4',
     lambda t: t['consecutive_losses'] >= 4),

    ('SHORT + (ATR < 0.9% OR Move < 1.0%)',
     lambda t: (t['direction'] == 'SHORT') & ((t['entry_atr_pct'] < 0.9) | (t['entry_move_size'] < 1.0))),

    ('SHORT + ATR < 1.0% + SigQual < 0',
     lambda t: (t['direction'] == 'SHORT') & (t['entry_atr_pct'] < 1.0) & (t['signal_quality'] < 0)),
]

print('\n' + '=' * 140)
print('FILTER TEST RESULTS')
print('=' * 140)
print(f"\n{'Filter':<50} {'Worst':<8} {'Winners':<10} {'Losers':<10} {'Total':<8} {'Score':<8}")
print('-' * 140)
...
for filter_name, filter_func in filters:
    ...
    try:
        ...
        score = (worst_removed / 20) * 100 - (winners_lost / len(winners)) * 50

        print(f"{filter_name:<50} {worst_removed:>2}/20   "
        ...
        print(f"{filter_name:<50} ERROR: {e}")
...
print(f"\nRemoves: {20 - best['worst_removed']} worst losers caught")
...
for i, f in enumerate(best_filters[:3], 1):
    print(f"\n{i}. {f['name']}")
    print(f"   Removes {f['worst_removed']}/20 worst | Keeps {f['winners_kept']}/{len(winners)} winners | {f['total_trades']} total trades | Score: {f['score']:.1f}")
