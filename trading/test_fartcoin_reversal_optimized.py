#!/usr/bin/env python3
"""Optimized FARTCOIN SHORT reversal strategy test - vectorized operations"""
import pandas as pd
import numpy as np
import sys
...
print(f"Loaded {len(df)} candles", file=sys.stderr)
sys.stderr.flush()
...
print("Indicators calculated", file=sys.stderr)
sys.stderr.flush()

def test_fast(rsi_trigger, limit_atr_offset, tp_pct):
    lookback = 5
    max_wait_bars = 20
    equity = 100.0
    trades = []
    ...
    for i in range(lookback, len(df)):
        ...
        if limit_pending:
            ...
            if row['high'] >= limit_price:
                ...
                future_highs = df.iloc[i+1:end_idx]['high'].values
                future_lows = df.iloc[i+1:end_idx]['low'].values

                for idx in range(len(future_highs)):
                    if future_highs[idx] >= sl_price:
                        hit_sl = True
                        break
                    elif future_lows[idx] <= tp_price:
                        hit_tp = True
                        break
    ...
    equity_curve = np.zeros(len(trades_df) + 1)
    equity_curve[0] = 100.0
    for idx, pnl in enumerate(trades_df['pnl_dollar'].values):
        equity_curve[idx + 1] = equity_curve[idx] + pnl
    ...
    winners = (trades_df['pnl_dollar'] > 0).sum()
...
print("\nTesting 120 configurations...", file=sys.stderr)
sys.stderr.flush()
results = []
config_count = 0
for rsi in [68, 70, 72, 74, 76]:
    for offset in [0.4, 0.6, 0.8, 1.0]:
        for tp in [5, 6, 7, 8, 9, 10]:
            config_count += 1
            r = test_fast(rsi, offset, tp)
            if r:
                results.append(r)
            if config_count % 30 == 0:
                print(f"  {config_count}/120 configs completed...", file=sys.stderr)
                sys.stderr.flush()

print(f"\nComplete! Found {len(results)}/120 valid configs", file=sys.stderr)
sys.stderr.flush()
