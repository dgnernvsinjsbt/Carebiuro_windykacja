#!/usr/bin/env python3
"""
Test LONG reversal strategy on NASDAQ-100
Buy oversold breakouts instead of shorting overbought
"""
import pandas as pd
import numpy as np

print("="*90)
print("NASDAQ-100 LONG REVERSAL STRATEGY - PARAMETER OPTIMIZATION")
print("="*90)
...
def backtest_config(df, rsi_trigger, limit_offset, tp_pct, lookback=5, max_wait=20, max_sl_pct=5.0):
    """
    Backtest LONG reversal configuration

    Entry: RSI < trigger (oversold) → wait for break ABOVE swing high → buy pullback
    """
    equity = 100.0
    trades = []
...
# RSI: Lower levels for oversold (30-40)
# Limit offset: How far below breakout to place limit
# TP: Target profit

configs = [
    # RSI 28 (very oversold)
    {'rsi': 28, 'offset': 0.15, 'tp': 1.5},
    {'rsi': 28, 'offset': 0.20, 'tp': 2.0},
    {'rsi': 28, 'offset': 0.25, 'tp': 2.5},

    # RSI 30 (oversold)
    {'rsi': 30, 'offset': 0.15, 'tp': 1.5},
    {'rsi': 30, 'offset': 0.20, 'tp': 2.0},
    {'rsi': 30, 'offset': 0.25, 'tp': 2.5},
    {'rsi': 30, 'offset': 0.30, 'tp': 3.0},

    # RSI 32 (medium oversold)
    {'rsi': 32, 'offset': 0.15, 'tp': 2.0},
    {'rsi': 32, 'offset': 0.20, 'tp': 2.5},
    {'rsi': 32, 'offset': 0.25, 'tp': 3.0},

    # RSI 35 (light oversold)
    {'rsi': 35, 'offset': 0.15, 'tp': 2.0},
    {'rsi': 35, 'offset': 0.20, 'tp': 2.5},
    {'rsi': 35, 'offset': 0.25, 'tp': 3.0},
    {'rsi': 35, 'offset': 0.30, 'tp': 3.5},

    # RSI 38 (barely oversold)
    {'rsi': 38, 'offset': 0.15, 'tp': 2.0},
    {'rsi': 38, 'offset': 0.20, 'tp': 2.5},
    {'rsi': 38, 'offset': 0.25, 'tp': 3.0},

    # RSI 40 (neutral-ish)
    {'rsi': 40, 'offset': 0.20, 'tp': 2.0},
    {'rsi': 40, 'offset': 0.25, 'tp': 2.5},
    {'rsi': 40, 'offset': 0.30, 'tp': 3.0},
]

print(f"\n🔄 Testing {len(configs)} LONG reversal configurations...")
print()
...
# Display results
print("\n" + "="*90)
print("🏆 TOP 20 NASDAQ-100 LONG REVERSAL CONFIGS (by Return/DD)")
print("="*90)
print()
...
print("="*90)
print("🎯 BEST LONG REVERSAL CONFIGURATION")
print("="*90)
print()
print(f"Parameters:")
...
# Comparison
print("="*90)
print("📊 LONG vs SHORT REVERSAL COMPARISON")
print("="*90)
print()
print(f"{'Metric':<20} | {'SHORT (failed)':<20} | {'LONG (best)':<20}")
print("-" * 65)
print(f"{'Direction':<20} | {'SHORT overbought':<20} | {'LONG oversold':<20}")
print(f"{'RSI Trigger':<20} | {'>68':<20} | {'<' + str(int(best['rsi_trigger'])):<20}")
long_rdd = f"{best['return_dd']:.2f}x"
long_ret = f"{best['total_return']:+.1f}%"
print(f"{'Return/DD':<20} | {'-0.25x':<20} | {long_rdd:<20}")
print(f"{'Total Return':<20} | {'-17.3%':<20} | {long_ret:<20}")
print()

if best['return_dd'] > 0:
    print("✅ LONG REVERSAL WORKS on NASDAQ!")
    print(f"   While SHORT lost -17.3%, LONG gained {best['total_return']:+.1f}%")
else:
    print("❌ BOTH LONG and SHORT reversals struggle on NASDAQ in this period")

# Save results
results_df.to_csv('nasdaq_long_reversal_results.csv', index=False)
print("\n💾 Full results saved to: nasdaq_long_reversal_results.csv")
print("="*90)
