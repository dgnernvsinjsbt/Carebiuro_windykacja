...
for (const client of windykacjaClients) {
  ...
  const qualifyingInvoices = allInvoices.filter(inv => {
    ...
    // Check if not sent OR sent long ago
    // Parse internal_note for EMAIL_1, EMAIL_2, EMAIL_3 flags
    const hasEmail1 = inv.internal_note && inv.internal_note.includes('[EMAIL_1]true');
    const hasEmail2 = inv.internal_note && inv.internal_note.includes('[EMAIL_2]true');
    const hasEmail3 = inv.internal_note && inv.internal_note.includes('[EMAIL_3]true');
    ...
    if (hasEmail1 && !hasEmail2 && inv.sent_time) {
      const sentDate = new Date(inv.sent_time);
    ...
    if (hasEmail2 && !hasEmail3 && inv.sent_time) {
      const sentDate = new Date(inv.sent_time);
      const daysSinceSent = Math.floor((Date.now() - sentDate.getTime()) / (1000 * 60 * 60 * 24));
      if (daysSinceSent >= 7) return true;
    }
...
clientsWithUnsent.forEach((item, idx) => {
  ...
  invoices.forEach((inv, i) => {
    ...
    if (hasEmail3) emailStatus = 'EMAIL_3 ✅ (czeka na kolejny)';
    else if (hasEmail2) emailStatus = 'EMAIL_2 ✅ (czeka na EMAIL_3)';
    else if (hasEmail1) emailStatus = 'EMAIL_1 ✅ (czeka na EMAIL_2)';

    let reason = '';
    if (!hasEmail1 && !hasEmail2 && !hasEmail3) {
      reason = '→ Gotowe do EMAIL_1 (pierwsze przypomnienie)';
    } else if (hasEmail1 && !hasEmail2 && inv.sent_time) {
      const sentDate = new Date(inv.sent_time);
      const daysSinceSent = Math.floor((Date.now() - sentDate.getTime()) / (1000 * 60 * 60 * 24));
      reason = `→ Gotowe do EMAIL_2 (${daysSinceSent} dni od EMAIL_1)`;
    } else if (hasEmail2 && !hasEmail3 && inv.sent_time) {
      const sentDate = new Date(inv.sent_time);
      const daysSinceSent = Math.floor((Date.now() - sentDate.getTime()) / (1000 * 60 * 60 * 24));
      reason = `→ Gotowe do EMAIL_3 (${daysSinceSent} dni od EMAIL_2)`;
    }
