/**
 * API Endpoint: Regenerate List Polecony Documents (PDF + Excel + ZIP)
 *
 * POST /api/list-polecony/regenerate
 * Body: { clientIds: number[] }
 *
 * Regeneruje dokumenty BEZ aktualizacji flagi [LIST_POLECONY] w Fakturowni.
 * Używane w zakładce "Wysłane" do ponownego wygenerowania dokumentów.
 *
 * Generuje:
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[ListPolecony Regenerate] Otrzymano request z clientIds:', clientIds);
    ...
    if (templateError) {
      console.warn('[Regenerate] Nie znaleziono szablonu listu poleconego - używam domyślnego:', templateError);
    }

    const letterTemplate = templateData || undefined;

    // Stwórz katalog tymczasowy dla plików
    const tempDir = path.join(os.tmpdir(), `list-polecony-regen-${Date.now()}`);
    ...
    try {
      ...
      const pdfPromises = clients.map(async (client, index) => {
        ...
        console.log(`[Regenerate] Wygenerowano PDF: ${pdfFilename} dla klienta ${client.name}`);
      ...
      console.log('[Regenerate] Wygenerowano plik Excel');
      ...
      console.log('[Regenerate] Utworzono archiwum ZIP');
      ...
      console.log('[Regenerate] Usunięto pliki tymczasowe');

      // BRAK aktualizacji flag - to jest różnica od /generate
    ...
    console.error('[Regenerate] Błąd generowania dokumentów:', error);
    return NextResponse.json(
      {
        success: false,
