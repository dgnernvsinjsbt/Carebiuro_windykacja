#!/usr/bin/env python3
"""
Test XLM-USDT with DOGE SHORT Reversal parameters
"""
import pandas as pd
import numpy as np

print("="*90)
print("XLM-USDT SHORT REVERSAL - DOGE PARAMETERS")
print("="*90)
...
print(f"\n🐕 DOGE Parameters Applied to XLM:")
...
print(f"\n" + "="*90)
print("🏆 XLM BACKTEST RESULTS (DOGE PARAMETERS)")
print("="*90)
print()
...
# Compare to DOGE
print(f"\n" + "="*90)
print("📊 XLM vs DOGE COMPARISON")
print("="*90)
print()

doge_stats = {
    'trades': 79,
    'contribution': 2993404,
    'months': '5/7 profitable',
    'best_trade': 998362
}

print(f"{'Metric':<25} | {'DOGE (backtest)':<20} | {'XLM (90 days)':<20}")
print("-" * 70)
print(f"{'Total Trades':<25} | {doge_stats['trades']:<20} | {len(trades_df):<20}")
print(f"{'Win Rate':<25} | {'88.9%':<20} | {f'{win_rate:.1f}%':<20}")
print(f"{'Return/DD':<25} | {'(part of 78,973x)':<20} | {f'{return_dd:.2f}x':<20}")
print(f"{'Total Return':<25} | {'+5,204,473% (portfolio)':<20} | {f'{total_return:+.1f}%':<20}")

print(f"\n💡 Volatility Context:")
xlm_atr = df['atr'].mean() / df['close'].mean() * 100
print(f"   DOGE ATR%: ~0.8-1.0% (estimated)")
print(f"   XLM ATR%:  {xlm_atr:.3f}%")
print(f"   Ratio:     {xlm_atr / 0.9:.2f}x")

# Save trades
trades_df.to_csv('xlm_doge_params_trades.csv', index=False)
print(f"\n💾 Trades saved to: xlm_doge_params_trades.csv")
print("="*90)
